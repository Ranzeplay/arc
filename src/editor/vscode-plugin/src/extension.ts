...
export function activate(context: vscode.ExtensionContext) {
    ...
    const diagnosticsCollection = vscode.languages.createDiagnosticCollection('Arc');
...
async function tryRegenerateDocumentInfo(document: vscode.TextDocument, diagnosticsProvider: ArcDiagnosticsProvider) {
    if (document.languageId === 'arc') {
        diagnosticsProvider.updateDiagnostics(document, vscode.languages.createDiagnosticCollection('Arc'));
    }
}
