...
import {
	Arc_function_blockContext,
	Arc_namespace_blockContext,
	Arc_statementContext,
	Arc_stmt_declContext,
	ArcSourceCodeParser,
} from './ArcSourceCodeParser';
...
class SymbolExtractor implements ArcSourceCodeParserListener {
	...
	enterArc_namespace_block(ctx: Arc_namespace_blockContext) {
		const fullName = ctx
			.arc_namespace_declarator()
			.arc_namespace_identifier().text;
		...
		for (const member of ctx.arc_namespace_member()) {
			if (member.arc_enum_declarator()) {
					...
					for (const enumMember of enumCtx.arc_enum_member()) {
						if (enumMember.arc_single_identifier()) {
							...
							members.push({
								...
								range: {
									start: {
										line: memberStart.line - 1,
										character: memberStart.charPositionInLine,
									},
									end: {
										line: memberStop!.line - 1,
										character:
											memberStop!.charPositionInLine + memberStop!.text!.length,
									},
								},
							});
						}
					...
					enums.push({
						...
						range: {
							start: {
								...
								line: stop!.line - 1,
								character: stop!.charPositionInLine + stop!.text!.length,
							},
						},
						children: members,
					});
				...
				if (groupCtx.arc_single_identifier()) {
					...
					for (const members of groupCtx
						.arc_wrapped_group_member()
						.arc_group_member()) {
						if (members.arc_group_function()?.arc_function_block()) {
							const funcCtx = members
								.arc_group_function()!
								.arc_function_block()!;
							const funcSymbol = parseFunction(funcCtx);
							if (funcSymbol) {
								if (
									funcCtx
										...
										?.arc_wrapped_arg_list()
										?.arc_arg_list()
										?.arc_self_data_declarator()
								) {
							...
							if (
								fieldCtx.arc_data_declarator() &&
								fieldCtx.arc_data_declarator().arc_single_identifier()
							) {
								const fieldName = fieldCtx
									.arc_data_declarator()
									.arc_single_identifier().text;
								...
								fields.push({
									...
									range: {
										start: {
											line: fieldStart.line - 1,
											character: fieldStart.charPositionInLine,
										},
										end: {
											line: fieldStop!.line - 1,
											character:
												fieldStop!.charPositionInLine + fieldStop!.text!.length,
										},
									},
								});
							}
							...
							lifecycleFunctions.push({
								...
								range: {
									start: {
										line: lifeStart.line - 1,
										character: lifeStart.charPositionInLine,
									},
									end: {
										line: lifeStop!.line - 1,
										character:
											lifeStop!.charPositionInLine + lifeStop!.text!.length,
									},
								},
							});
						}
...
function parseFunction(ctx: Arc_function_blockContext): Symbol | null {
	if (ctx.arc_function_declarator()) {
		...
		if (
			funcDeclCtx.arc_wrapped_arg_list()?.arc_arg_list()?.arc_data_declarator()
		) {
			for (const param of funcDeclCtx
				.arc_wrapped_arg_list()!
				.arc_arg_list()!
				.arc_data_declarator()!) {
				...
				funcArgs.push({
					...
					range: {
						start: {
							line: paramStart.line - 1,
							character: paramStart.charPositionInLine,
						},
						end: {
							line: paramStop!.line - 1,
							character:
								paramStop!.charPositionInLine + paramStop!.text!.length,
						},
					},
				});
			}
		...
		let innerSymbols = parseStatements(
			ctx.arc_wrapped_function_body().arc_statement()
		);
...
function parseStatements(ctx: Arc_statementContext[]): Symbol[] {
	...
	for (const stmt of ctx) {
		if (stmt.arc_stmt_decl()) {
			...
			const innerSymbols = parseStatements(
				whileCtx.arc_wrapped_function_body().arc_statement()
			);
			...
			const innerSymbols = parseStatements(
				forCtx.arc_wrapped_function_body().arc_statement()
			);
			if (leadingSymbol) {
			...
			const innerSymbols = parseStatements(
				ifCtx.arc_wrapped_function_body().flatMap(b => b.arc_statement())
			);
			symbols.push(...innerSymbols);
		}
	}
...
function parseDataDeclarator(ctx: Arc_stmt_declContext): Symbol | null {
	if (
		ctx.arc_data_declarator() &&
		ctx.arc_data_declarator().arc_single_identifier()
	) {
