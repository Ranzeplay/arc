...
{
    public virtual uint Id { get => ManualId ?? GeneratedId; set => ManualId = value; }

    private uint? ManualId { get; set; } = null;

    private uint GeneratedId => HashHelper.GetDjb2Hash(Signature);
    ...
    public IEnumerable<string> ParentSignatureStack
    {
        get
        {
            ...
            var current = this;
            while (current is not null && current is not ArcRootScopeTreeNode)
            {
    ...
    public ArcRootScopeTreeNode Root
    {
        get
        {
            var current = this;
            while (current is not ArcRootScopeTreeNode)
            {
                current = current.Parent;
            }
            return (ArcRootScopeTreeNode)current;
        }
    }
